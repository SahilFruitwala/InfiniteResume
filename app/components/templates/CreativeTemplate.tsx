...
export const CreativeTemplate = ({ data }: { data: ResumeData }) => {
  ...
  const accentColor = theme?.creative?.accentColor ?? theme?.accentColor ?? '#ec4899'; // pink-500
          ...
          <section key="experience" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${itemGap}px` }}>
              {data.experience.map((exp) => (
                <div key={exp.id} className="relative z-10 pl-6 border-l-2 border-gray-100 pb-2">
                  <div className="absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white" style={{ backgroundColor: accentColor }}></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1">
                    <h3 className="font-extrabold text-gray-900" style={{ fontSize: `${typography?.fontSizeItemHeading ?? 18}px` }}>{exp.position}</h3>
                    <span className="text-[0.85em] font-bold px-3 py-1 rounded-sm uppercase tracking-wider mt-1 sm:mt-0" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>
                      {exp.startDate} – {exp.endDate}
                    </span>
                  </div>
                  <div className="text-[1.05em] font-medium text-gray-800 mb-2">{exp.company}</div>
                  <div 
          ...
          <section key="education" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${itemGap}px` }}>
              {data.education.map((edu) => (
                <div key={edu.id} className="relative z-10 pl-6 border-l-2 border-gray-100 pb-2">
                  <div className="absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white" style={{ backgroundColor: accentColor }}></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1">
                    <h3 className="font-extrabold text-gray-900" style={{ fontSize: `${typography?.fontSizeItemHeading ?? 18}px` }}>{edu.degree}</h3>
                    <span className="text-[0.85em] font-bold px-3 py-1 rounded-sm uppercase tracking-wider mt-1 sm:mt-0" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>
                      {edu.startDate} – {edu.endDate}
                    </span>
                  </div>
                  <div className="text-[1.05em] font-medium text-gray-800">{edu.institution}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section key="projects" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${itemGap}px` }}>
              {data.projects.map((proj) => (
                <div key={proj.id} className="relative z-10 pl-6 border-l-2 border-gray-100 pb-2">
                  <div className="absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white" style={{ backgroundColor: accentColor }}></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1">
                    <h3 className="font-extrabold text-gray-900" style={{ fontSize: `${typography?.fontSizeItemHeading ?? 18}px` }}>{proj.name}</h3>
                    {proj.link && (
                      <a href={proj.link} target="_blank" rel="noreferrer" className="text-[0.9em] font-bold underline mt-1 sm:mt-0" style={{ color: accentColor }}>
          ...
          <section key="volunteerWork" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${itemGap}px` }}>
              {data.volunteerWork.map((vol) => (
                <div key={vol.id} className="relative z-10 pl-6 border-l-2 border-gray-100 pb-2">
                  <div className="absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white" style={{ backgroundColor: accentColor }}></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1">
                    <h3 className="font-extrabold text-gray-900" style={{ fontSize: `${typography?.fontSizeItemHeading ?? 18}px` }}>{vol.position}</h3>
                    <span className="text-[0.85em] font-bold px-3 py-1 rounded-sm uppercase tracking-wider mt-1 sm:mt-0" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>
                      {vol.startDate} – {vol.endDate}
                    </span>
                  </div>
                  <div className="text-[1.05em] font-medium text-gray-800 mb-2">{vol.organization}</div>
                  <div 
                    className="leading-relaxed text-gray-600 whitespace-pre-line [&_b]:font-bold [&_strong]:font-bold [&_i]:italic [&_em]:italic [&_u]:underline [&_ul]:list-disc [&_ul]:pl-4 [&_li]:mb-[var(--bullet-gap)] [&_ul]:my-[var(--list-margin)]"
          ...
          <section key="awards" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${itemGap}px` }}>
              {data.awards.map((award) => (
                <div key={award.id} className="relative z-10 pl-6 border-l-2 border-gray-100 pb-2">
                  <div className="absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white" style={{ backgroundColor: accentColor }}></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1">
                    <h3 className="font-extrabold text-gray-900" style={{ fontSize: `${typography?.fontSizeItemHeading ?? 18}px` }}>{award.name}</h3>
                    <span className="text-[0.85em] font-bold px-3 py-1 rounded-sm uppercase tracking-wider mt-1 sm:mt-0" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>{award.date}</span>
                  </div>
                  <div className="text-[1.05em] font-medium text-gray-800 mb-1">{award.issuer}</div>
                  <div 
                    className="leading-relaxed text-gray-600 whitespace-pre-line [&_b]:font-bold [&_strong]:font-bold [&_i]:italic [&_em]:italic [&_u]:underline [&_ul]:list-disc [&_ul]:pl-4 [&_li]:mb-[var(--bullet-gap)] [&_ul]:my-[var(--list-margin)]"
          ...
          <section key="languages" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div className="flex flex-wrap gap-4 text-gray-800 leading-relaxed">
              {data.languages.map(lang => (
                <div key={lang.id} className="bg-gray-100 px-4 py-2 rounded-lg font-medium">
          ...
          <section key="interests" style={{ marginBottom: `${sectionGap}px` }}>
            ...
            <div className="flex flex-wrap gap-3">
              {data.interests.map(i => (
                <span key={i.id} className="font-bold tracking-wide text-white px-4 py-1.5 rounded-full" style={{ backgroundColor: accentColor }}>
                  {i.name}
                </span>
              ))}
            </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ paddingTop: `${pageMarginTop}px`, paddingBottom: `${pageMarginBottom}px` }} className="px-12 relative z-10">
        <header className="mb-12 flex flex-col md:flex-row gap-8 items-center md:items-start pb-8 border-b-2 border-gray-100" style={{ marginBottom: `${sectionGap}px` }}>
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-2 mt-4 text-[0.95em] font-medium text-gray-600">
              ...
              {data.personalInfo.website && (
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full" style={{ backgroundColor: accentColor }}></span>
                  <a href={data.personalInfo.website.startsWith('http') ? data.personalInfo.website : `https://${data.personalInfo.website}`} target="_blank" rel="noreferrer" className="hover:underline" style={{ color: accentColor }}>
              ...
              {data.socialLinks && data.socialLinks.length > 0 && data.socialLinks.map(link => (
                <div key={link.id} className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full" style={{ backgroundColor: accentColor }}></span>
                  <a href={link.url.startsWith('http') ? link.url : `https://${link.url}`} target="_blank" rel="noreferrer" className="hover:underline" style={{ color: accentColor }}>
                    {link.name}
                  </a>
                </div>
              ))}
