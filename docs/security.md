...
### 3. API key storage and encryption

**Issue**

1. BYOK API keys were persisted in `localStorage` as plain text.
2. If XSS occurred, those keys were easy to exfiltrate from storage.

**What I changed**

1. Implemented **AES-256-GCM encryption** for API keys stored in `localStorage` using the Web Crypto API.
2. Keys are now encrypted using a per-user derived key (Clerk `userId` + PBKDF2).
3. Provider preference is persisted separately.
4. Full details on the encryption scheme are documented in [api-key-security.md](api-key-security.md).
5. Updated settings panel messaging to reflect the new behavior:
...
1. Protect keys against casual inspection and cross-user access on shared devices.
2. Minimize secret exposure in browser storage while maintaining user convenience.
3. Decrypt keys only when needed and keep them in a short-lived in-memory cache for the active session.
