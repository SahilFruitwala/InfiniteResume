...
export const ModernTemplate = memo(
  ({ data, isDark }: { data: ResumeData; isDark?: boolean }) => {
    ...
    */
              ...
              >
                {data.education.map((edu) => (
                  <div key={edu.id}>
                    ...
                    <div
                      className={`text-[1.05em] font-bold ${isDark ? "text-slate-100" : "text-slate-900"} mb-1`}
                    >
                      {edu.institution}
                    </div>
      ...
      switch (sectionId) {
        ...
        case "volunteerWork":
          return data.volunteerWork && data.volunteerWork.length > 0 ? (
            ...
            >
              <h2
                className={`font-bold uppercase tracking-wider ${isDark ? "text-slate-100" : "text-slate-800"} flex items-center gap-3`}
                style={{
                  fontSize: `${typography?.fontSizeSectionHeading || 18}px`,
                  marginBottom: `${sectionTitleGap}px`,
                }}
    ...
    return (
      <div
        className={`flex max-w-4xl mx-auto w-full min-h-[1056px] print:!min-h-0 shadow-sm print:!shadow-none print:!bg-white print:!text-gray-900 ${isDark ? "bg-[#111111] text-slate-100" : "bg-white text-gray-800"}`}
        style={
          {
      ...
      >
        ...
        <div
          data-print-wrapper
          className={`w-1/3 transition-colors duration-300 ${isDark ? "bg-[#111111]" : "bg-slate-800"} text-white px-8 flex flex-col gap-8`}
          style={{
        ...
        <div
          data-print-wrapper
          className={`w-2/3 px-8 transition-colors duration-300 print:!bg-white print:!text-gray-800 ${isDark ? "bg-[#111111] text-slate-100" : "bg-slate-50 text-gray-800"}`}
          style={{
            paddingTop: `${pageMarginTop}px`,
            paddingBottom: `${pageMarginBottom}px`,
          }}
        >
          {sortedRightSections.map((sectionId) =>
            renderRightSection(sectionId),
          )}
        </div>
      </div>
    );
